			...
			return {
				on: {
					...
					init      : function () {
						...
						qodef.body.trigger(
							'cuthbert_trigger_swiper_is_initialized',
							[$holder, options]
						);
...
(function ($) {
	...
	var qodefWooButtons = {
		init: function () {
			var $button = $('.qodef-woo-product-image-inner .button, .qodef-woo-product-image-inner .added_to_cart');
				...
				$button.each(
					function () {
						var $thisButton = $(this),
							...
							buttonContent = '<span class="qodef-m-text">' + buttonText + '</span>';

						$thisButton.empty().append(buttonContent);
						$thisButton.addClass('qodef-shortcode qodef-m  qodef-button qodef-layout--textual  qodef-html--link');
						$thisButton.wrap('<div class="qodef-woo-button-wrapper"></div>');
					}
				);
			}

			qodef.body.on(
				'added_to_cart wc_cart_emptied',
				function () {
					qodefWooButtons.init();
				}
			);
