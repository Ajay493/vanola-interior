...
if ( ! class_exists( 'Cuthbert_Welcome_Page' ) ) {
	/**
	 * Welcome page theme class
	 */
	class Cuthbert_Welcome_Page {
		private static $instance;

		/**
		 * @return Cuthbert_Welcome_Page
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_cuthbert_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_cuthbert_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_cuthbert_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'cuthbert_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			add_theme_page(
				esc_html__( 'About', 'cuthbert' ),
				esc_html__( 'About', 'cuthbert' ),
				current_user_can( 'edit_theme_options' ),
				'cuthbert_welcome_page',
				array( $this, 'welcome_page_content' )
			);

			remove_submenu_page( 'themes.php', 'cuthbert_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( CUTHBERT_ROOT_DIR . '/screenshot.png' ) ? CUTHBERT_ROOT . '/screenshot.png' : CUTHBERT_ROOT . '/screenshot.jpg';

			cuthbert_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			...
			if ( 'appearance_page_cuthbert_welcome_page' === $hook ) {
				wp_enqueue_style( 'cuthbert-welcome-page-style', CUTHBERT_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}

	Cuthbert_Welcome_Page::get_instance();
}
